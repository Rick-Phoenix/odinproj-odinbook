...
export const MockChat = ({ messages }: { messages: string[] }) => {
  ...
  return (
    <div ref={containerRef} className="relative my-32 h-[50vh] w-full overflow-y-auto px-8 scrollbar-hidden">
      <div className="grid-4 justify-items-center gap-x-28" data-aos="fade-up" data-aos-duration="1000">
        ...
        <div className="relative row-span-2 row-start-1 h-[150vh] justify-self-end">
          <motion.div className="sticky grid auto-rows-auto grid-cols-2 gap-y-8 px-5 center-y">
            {messages.map((message, index) => {
              ...
              const gridColumn = isEven ? "2/3" : "1/2";
              const justifySelf = isEven ? "end" : "start";
              const gridRowStart = index + 1;
              const msgType = isEven ? "message-user rounded-tr-none" : "message-contact rounded-tl-none ";
              return (
                <motion.div
                  key={index}
