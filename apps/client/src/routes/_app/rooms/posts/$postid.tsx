...
function renderRootComments(comments: Comments) {
  return comments.map((comment, i) => {
    ...
    const startingRow = previousCommentHadChildren ? i + 2 : i + 1;
    return (
      <>
        <div
          key={i}
          className={`col-start-1 col-end-3 row-start-${startingRow} grid grid-cols-[auto_1fr] items-center justify-self-start`}
        >
        ...
        {currentCommentChildren ? (
          <>
            <Separator
              orientation="vertical"
              className={`row-start-${startingRow + 1} justify-self-center`}
            />

            <div
              className={`col-start-2 row-start-${startingRow + 1} grid grid-cols-[auto_1fr] items-center pt-4`}
            >
              <div className="relative col-start-1 row-start-1 h-10 w-10 rounded-full bg-foreground">
                <span className="absolute -left-5 top-2 h-3 w-5 rounded-xl rounded-r-none rounded-t-none border-b border-l bg-transparent hover:border-b-white hover:border-l-white" />
              </div>
